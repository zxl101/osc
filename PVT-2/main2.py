...
from engine import train_one_epoch_jigsaw, evaluate
...
from loader import JigCluTransform, GaussianBlur
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='./checkpoints/jigsaw',
...
def main(args):
    ...
    train_transform = JigCluTransform(train_transform, 0)
    ...
    labeled_eval_loader = DataLoader(datasets['val'], batch_size=args.batch_size,
                                     shuffle=False, sampler=None, num_workers=args.num_workers)
    unlabeled_eval_loader = DataLoader(datasets['aux'], batch_size=args.batch_size,
                                       shuffle=False, sampler=None, num_workers=args.num_workers)
    open_set_loader = DataLoader(datasets['test_unknown'], batch_size=args.batch_size,
                                 shuffle=False, sampler=None, num_workers=args.num_workers)
    ...
    fea_len = 512
    fc_clu = nn.Linear(fea_len//4, args.batch_size).to(device)
    fc_loc = nn.Linear(fea_len//4, 4).to(device)
    ...
    # args,output_dir = "./checkpoints/jigsaw/"
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_jigsaw(
        ...
        if args.output_dir:
            checkpoint_paths = [output_dir / 'jigsaw_checkpoint.pth']
        ...
        if args.output_dir and utils.is_main_process():
            with (output_dir / "jigsaw_log.txt").open("a") as f:
