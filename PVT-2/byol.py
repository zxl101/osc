...
from engine import train_one_epoch_byol, evaluate_byol
...
def get_args_parser():
    ...
    parser.add_argument('use_momentum', action='store_true', default=False, help='if true, use BYOL; if false, use SimSiam')
...
def main(args):
    ...
    #Logger
    args.output_dir = os.path.join(args.output_dir,args.dataset, 'pretrain')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_byol(
        ...
        _ = evaluate_byol(aux_test_loader, model, device, epoch, logger, name="Aux")
        _ = evaluate_byol(test_unknown_loader, model, device, epoch, logger, name="Unknown")
        _ = evaluate_byol(data_loader_val, model, device, epoch, logger)
        # print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
