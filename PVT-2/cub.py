...
cub_root = '/media/neuron/Elements/Dataset/CUB'
...
def get_cub_datasets(train_transform, test_transform, train_classes=range(100), auxiliary_classes=range(100,150),
                       open_set_classes=range(150, 200), balance_open_set_eval=False, split_train_val=True, seed=0):
    ...
    if auxiliary_classes is not None:
        ...
        aux_dataset_whole = CustomCub2011(root=cub_root, transform=test_transform, train=False)
        ...
        mix_dataset_train = subsample_classes(train_dataset_whole3, include_classes=range(150))
        train_dataset_whole4 = CustomCub2011(root=cub_root, transform=test_transform, train=False)
        mix_dataset_test = subsample_classes(train_dataset_whole4, include_classes=range(150))
...
def get_cub_loaders(train_classes=range(100), auxiliary_classes=range(100,150),
    ...
    image_size = 224
    train_transform = transforms.Compose([transforms.Resize((image_size, image_size), Image.BILINEAR),
                                         transforms.RandomCrop((image_size, image_size)),
                                         transforms.RandomHorizontalFlip(),
                                         transforms.ToTensor(),
                                         transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
    test_transform = transforms.Compose([transforms.Resize((image_size, image_size), Image.BILINEAR),
