...
from .cifarloader import CIFAR10, CIFAR100
from .dogloader import DOGS
from .data.cub import CustomCub2011, get_cub_datasets
...
class GenericDataset(data.Dataset):
    def __init__(self, dataset_name, split, random_sized_crop=False,
                 num_imgs_per_cat=None, dataset_root=None, num_labeled_classes=5):
        ...
        if self.dataset_name=='cifar10':
            ...
            self.data = CIFAR10(dataset_root, split=self.split, transform=self.transform, target_list=list(range(num_labeled_classes)), ssl=True)
            ...
            #     dataset_root, train=self.split=='train',
            #     download=True, transform=self.transform)
            self.data = CIFAR100(dataset_root, split=self.split, transform=self.transform,
                                target_list=list(range(num_labeled_classes)), ssl=True)
        elif self.dataset_name=='dog_stanford':
            self.mean_pix = [x / 255.0 for x in [125.3, 123.0, 113.9]]
            self.std_pix = [x / 255.0 for x in [63.0, 62.1, 66.7]]

            transform = []
            if (split != 'test'):
                transform.append(transforms.RandomResizedCrop(224, ratio=(1, 1.3)))
                transform.append(transforms.RandomHorizontalFlip())
            else:
                transform.append(transforms.Resize((224,224)))
            transform.append(lambda x: np.asarray(x))
            self.transform = transforms.Compose(transform)
            self.data = DOGS(dataset_root, split=self.split, transform=self.transform,target_list=range(20))
        elif self.dataset_name=='cub':
            self.mean_pix = [x / 255.0 for x in [125.3, 123.0, 113.9]]
            self.std_pix = [x / 255.0 for x in [63.0, 62.1, 66.7]]
            transform = []
            if (split != 'test'):
                transform.append(transforms.RandomResizedCrop((448,448)))
                transform.append(transforms.RandomHorizontalFlip())
            else:
                transform.append(transforms.Resize((448, 448)))
            transform.append(lambda x: np.asarray(x))
            self.transform = transforms.Compose(transform)
            datasets = get_cub_datasets(train_transform=self.transform, test_transform=self.transform,train_classes=range(150),auxiliary_classes=None,open_set_classes=range(150,200))
            if split != 'test':
                self.data = datasets['train']
            else:
                self.data = datasets['test_known']
    ...
    def get_iterator(self, epoch=0):
        ...
        if self.unsupervised:
            ...
            def _load_function(idx):
                ...
                img0, class_label = self.dataset[idx]
                # print(img0.shape)
                rotated_imgs = [
                    self.transform(img0),
                ...
                class_label = torch.LongTensor([class_label, class_label, class_label, class_label])
                return torch.stack(rotated_imgs, dim=0), rotation_labels, class_label
